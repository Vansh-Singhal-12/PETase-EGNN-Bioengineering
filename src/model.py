...
class PETaseStabilityEGNN(nn.Module):
    def __init__(self, in_dim=8, emb_dim=32, dropout=0.1):
        ...
        # CHANGED: regression head now outputs [mu, log_var] instead of a
        # single scalar. mu is the point prediction (same physical meaning
        # as before); log_var parameterizes predictive uncertainty, trained
        # via Gaussian NLL in train.py rather than plain MSE.
    ...
    def forward(self, graph_data, mutation_pos, use_interaction=True):
        ...
        mutated_h_per_pos = h[pos_list]
        mutated_node_h_sum = mutated_h_per_pos.sum(dim=0)

        k = mutated_h_per_pos.size(0)
        if k > 1 and use_interaction:
            h_i = mutated_h_per_pos.unsqueeze(1)
            h_j = mutated_h_per_pos.unsqueeze(0)
            pairwise_products = h_i * h_j

            pair_mask = torch.triu(torch.ones(k, k, dtype=torch.bool, device=h.device), diagonal=1)
            interaction_raw = pairwise_products[pair_mask].mean(dim=0)
            h_interact = self.interaction_mlp(interaction_raw)
        else:
            h_interact = torch.zeros(mutated_h_per_pos.size(-1), device=h.device)

        mutated_node_h = (mutated_node_h_sum + h_interact).unsqueeze(0)
        ...
        head_out = self.regression_head(combined_h)  # [1, 2]
        mu = head_out[:, 0]
        log_var = torch.clamp(head_out[:, 1], min=-10.0, max=10.0)

        return mu, log_var, node_preds