...
def test_e3_equivariance():
    ...
    with torch.no_grad():
        orig_mu, orig_log_var, _ = model(graph_data, pos)
        orig_val = orig_mu.item()
        orig_sigma = torch.exp(0.5 * orig_log_var).item()
        print(f"Original Prediction : mu={orig_val:.6f} °C, sigma={orig_sigma:.6f}\n")
        ...
        for trial in range(1, 51):
            ...
            trans_mu, trans_log_var, _ = model(transformed_graph, pos)
            diff = abs(orig_val - trans_mu.item())
            sigma_diff = abs(orig_sigma - torch.exp(0.5 * trans_log_var).item())
            max_diff_rotation = max(max_diff_rotation, diff)

            print(f"Trial {trial:2d}: mu diff {diff:.8f} | sigma diff {sigma_diff:.8f}")
            assert diff < 1e-4, f"E(3) Equivariance violated (mu)! Difference: {diff}"
            assert sigma_diff < 1e-4, f"E(3) Equivariance violated (sigma)! Difference: {sigma_diff}"

        print("-" * 68)
        print("PART 2: 50 trials -- reflections (O(3), det=-1) + translations")
        print("-" * 68)
        ...
        for trial in range(1, 51):
            ...
            print(f"Reflection {trial:2d}: mu diff {diff:.8f} | sigma diff {sigma_diff:.8f}")
            assert diff < 1e-4, f"E(3) Equivariance violated under reflection (mu)! Difference: {diff}"
            assert sigma_diff < 1e-4, f"E(3) Equivariance violated under reflection (sigma)! Difference: {sigma_diff}"

        print("-" * 68)
        print(f" Max |diff| across 50 rotation trials   : {max_diff_rotation:.8f}")
        print(f" Max |diff| across 50 reflection trials  : {max_diff_reflection:.8f}")
        print("-" * 68)
        print(" SUCCESS: Bit-for-bit E(3)-Equivariance numerically PROVED for BOTH mu and sigma!")
        print("=" * 68)
